...
$( document ).ready(function() {
	...
	(function(){
		...
		$(window).on('resize', function(){
			var left = parseInt($panel.css('left'), 10) || 0;
			var top = parseInt($panel.css('top'), 10) || 0;
			var maxLeft = $(window).width() - $panel.outerWidth();
			var maxTop = $(window).height() - $panel.outerHeight();
	...
	// Scale up the entire sidebar UI slightly for readability
	$('.sidebar-wrapper').css('transform', 'scale(1.15)');
	...
	$('#downloadWidth').val(3240);
	$('#downloadHeight').val(1440);
	...
	function setNewParams(){
		...
		smokyBG.data('waterpipe').setOption('seed', params.seed && params.seed.length ? params.seed : null);
		var ts = parseFloat(params.playbackSpeed);
		if (!isFinite(ts) || ts <= 0) ts = 1;
		smokyBG.data('waterpipe').setOption('timeScale', ts);
	...
	// Seed refresh
	$('.sidebar-wrapper').on('click', '.btn-seed-refresh', function(){
		$('#seed').val(Math.floor(Math.random()*0xFFFFFFFF));
		return false;
	});

	// Download as Video
	$('.sidebar-wrapper').on('click', '.btn-download-video', function(){
		setNewParams();
		var width = $('#downloadWidth').val(), height = $('#downloadHeight').val();
		var fps = 30; // default fps; could add input later
		smokyBG.data('waterpipe').downloadVideo(width, height, fps);
		return false;
	});
